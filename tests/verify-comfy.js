...
check('server 有中继取图接口', server.includes('function comfyImageData(') && server.includes("'/api/comfy-image'"));
...
check('Java 有 App 内缓存方法', java.includes('cacheImageToApp') && java.includes('files.length > 50'));
...
check('relay QoS1 发送与 PUBACK', relay.includes('_publish(topic, payload)') && relay.includes('_retryPublish(rec)') && relay.includes('_handlePuback(') && relay.includes('_sendPuback(') && relay.includes('packetId ? 2 : 0') && relay.includes('qos = (flags >> 1) & 3'));
check('server 中继取图分片+file-gone', server.includes('function comfyImageData(') && server.includes("error: 'file-gone'") && server.includes('chunkCount: total') && server.includes('if (params.meta)'));
check('server 图片延迟删除 .del', server.includes("f + '.del'") && server.includes("name.endsWith('.del')") && server.includes('2 * 60 * 60 * 1000'));
check('server RPC 回复耗时字节日志', server.includes("'ms 字节 ' + Buffer.byteLength"));
check('app 中继取图分片拉取', app.includes("apiCall('comfyImage', { path, meta: true }") && app.includes('index: idx') && app.includes('parts.join'));
check('app 取图失败自动重试', app.includes('function retryFetchComfyData(') && app.includes('[0, 1000, 3000, 8000]') && app.includes('原图已清理，无法恢复'));
...
check('server GitHub 图片资产清理', server.includes('function pruneGithubImages(') && server.includes("releases/assets/' + a.id") && server.includes('githubImgRetentionCount') && server.includes('24 * 60 * 60 * 1000'));
...
check('app GitHub 失败回退中继', app.includes('}).catch(() => retryFetchComfyData(img, path, 0));'));
check('app 取图并发 8 + 结果缓存', app.includes('Math.min(8, meta.chunkCount)') && app.includes('comfyDataResultCache') && app.includes('function comfyResultGet('));
...
check('app 图片映射按 200 条清理', app.includes('entries.length > 200') && app.includes('Object.fromEntries'));
